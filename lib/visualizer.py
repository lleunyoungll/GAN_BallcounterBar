...
class Visualizer():
    ...
    def save_current_images(self, epoch, reals, fakes, fixed):
        ...
        Args:
            ...
            fixed ([FloatTensor]): Fixed Fake Image
        """
        vutils.save_image(reals, '%s/reals_epoch%02d.png' %(self.img_dir, epoch+1),normalize=True)
        vutils.save_image(fakes, '%s/fakes_epoch%02d.png' %(self.img_dir, epoch+1), normalize=True)
        #vutils.save_image(fixed, '%s/fixed_fakes_epoch%03d.png' %(self.img_dir, epoch+1), normalize=True)
